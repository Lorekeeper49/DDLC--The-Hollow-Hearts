...
init python:
    ...
    class AnimatedMask(renpy.Displayable):
        ...
        def render(self, width, height, st, at):
            ...
            nr.operation = renpy.display.render.IMAGEDISSOLVE
            nr.operation_alpha = 1.0
            nr.operation_complete = self.oc + math.pow(math.sin(st * self.speed / 8), 64 * self.frequency) * self.amount
            nr.operation_parameter = self.op
            
            nr.blit(mb, (0, 0), focus=False, main=False)
            nr.blit(nr, (0, 0), focus=False, main=False)
            nr.blit(cr, (0, 0))
